...
describe('getViewStructure testing', () => {
  ...
  test('getViewStructure success', async () => {
    techTextAxiosClient.get.mockImplementation(() => Promise.resolve({ data: mockResolveData }))
    const resp = await getViewStructure()
    expect(techTextAxiosClient.get).toHaveBeenCalledTimes(1)
    expect(techTextAxiosClient.get).toHaveBeenCalledWith('/structure/list')
