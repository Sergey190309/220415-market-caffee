...
class Confirmation(Resource):
    '''
    Check whether confirmaion expired, already confirmed otherwice confirm user and
    set him role user.
    ...
    def get(cls, confirmation_id: str):
        ...
        if not _confirmation:
            return {
                'message': str(_("Requested confirmation has not been found."))}, 404
        if _confirmation.is_expired:
            return {'message': str(_("Requested confirmation already expired."))}, 400
            return
...
class ConfirmationByUser(Resource):
    ...
    def post(cls, user_id: int):
        ...
        if _user.is_valid:
            return {
                'message': str(_(
                    "User with id '%(user_id)s' have status '%(role_id)s' already.",
        ...
        try:
            ...
            return {
                'message': str(_(
                    "Email has been sent to '%(email)s'. Please check your mail box. "
