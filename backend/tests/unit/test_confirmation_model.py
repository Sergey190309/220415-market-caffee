...
def test_confirmaion_save_finds_delete(test_client, confirmaion_instance):
    ...
    assert confirmaion_instance.id == _confirmaion_by_user_id.id and\
        confirmaion_instance.id == _confirmaion_by_id.id and\
        confirmaion_instance.user_id == _confirmaion_by_user_id.user_id and\
        confirmaion_instance.user_id == _confirmaion_by_id.user_id and\
        confirmaion_instance.expire_at == _confirmaion_by_user_id.expire_at and\
        confirmaion_instance.expire_at == _confirmaion_by_id.expire_at and\
        confirmaion_instance.confirmed == _confirmaion_by_user_id.confirmed and\
        confirmaion_instance.confirmed == _confirmaion_by_id.confirmed
...
def test_confirmation_expiring(test_client, confirmaion_instance):
    ...
    assert not confirmaion_instance.expired

    # Forse expiring.
    confirmaion_instance.force_to_expire()
    _maturity = confirmaion_instance.expire_at - int(time())
    assert _maturity <= 0
    assert confirmaion_instance.expired
