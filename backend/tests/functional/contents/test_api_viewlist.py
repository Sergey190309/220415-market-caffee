...
# @pytest.mark.active
def test_contents_view_list_get_no_token(test_client, url_view_list):
    '''
    Wrong - logged admin can access to this info only.
    '''
    _resp = test_client.get(url_view_list)
...
def test_view_list_get_user(
    ...
    _resp = test_client.get(url_view_list, headers=headers)
    assert _resp.status_code == 401
    assert isinstance(_resp.json, Dict)
