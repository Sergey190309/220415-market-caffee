...
def test_components_kinds_post(
        client, url_component_kinds, component_kind_api_resp):
    resp = component_kind_api_resp
    # print('\ntest_components_kinds_post, resp.status_code ->', resp.status_code)
    ...
    resp = client.post(url_component_kinds, json=_components_kinds_same_id_json)
    # print('\ntest_components_kinds_post, resp.status_code ->', resp.status_code)
    ...
    for key in _components_kinds_json.keys():
        ...
        resp = client.post(url_component_kinds, json=_components_kinds_bad_json)
...
def test_components_kinds_get(
    ...
    resp = client.get(url_component_kinds, json=_search_components_kinds_json)
    # Insure component is correctly gotten:
    for key in _components_kinds_json.keys():
        assert _components_kinds_json[key] == resp.json['payload'][key]

    # hange some searching criterion fail to get 404 from API:
...
def test_components_kinds_put(
    ...
    resp = client.get(url_component_kinds, json=_get_component_kind_json)
    ...
    resp = client.put(url_component_kinds, json=_put_component_kind_wrong_json)
    assert resp.status_code == 404
    assert 'payload' not in resp.json.keys()

    # Create json or correction:
    ...
    # Correct object:
    resp = client.put(url_component_kinds, json=_put_component_kind_json)
...
def test_components_kinds_delete(
    ...
    # Create json or deleting and delete object from db:
    ...
    resp = client.get(
        url_component_kinds, json=_delete_component_kind_json)
    assert resp.status_code == 404
    # print('\ntest_components_kinds_delete, resp.json.keys ->', resp.json.keys())
    assert 'payload' not in resp.json.keys()